...
This study aims to address these gaps by developing a linear model to analyze the impact of a nation’s status, healthcare factors, and economic indicators on life expectancy. The response variable is Life Expectancy, while the predictors include Status (Developed vs. Developing), Diphtheria Immunization Coverage, Government Total Expenditure, Numbers of Doctors and Nurses per 10000, Schooling and BMI. The analysis seeks to quantify the influence of these variables, providing insights into the disparities in life expectancy and identifying factors that contribute to these differences.
...
**Body Mass Index (`BMI`)**: This variable captures the average body mass index of the population, calculated as weight in kilograms divided by the square of height in meters. It is an aggregate measure derived from population health surveys and recorded as a numeric value.
...
2. **Body Mass Index (`BMI`)**: This variable represents the average Body Mass Index of the entire population in a country. BMI is a widely used health indicator that reflects the prevalence of obesity or underweight populations. Extreme values of BMI can negatively impact life expectancy by contributing to chronic diseases such as diabetes and cardiovascular conditions. The BMI condition is depicted in @fig-bmi.

3. **Diphtheria Immunization Coverage (`Diphtheria`)**: This variable represents the percentage of one-year-olds who have received the Diphtheria, Tetanus, and Pertussis (DTP3) immunization. Immunization coverage(@fig-diphtheria) is an important health indicator, as it reflects the effectiveness of a country's healthcare system in preventing infectious diseases, which in turn affects life expectancy.

4. **Schooling (`Schooling`)**: The number of years of schooling in a country is used as a proxy for education levels. Higher education levels typically correlate with better health outcomes and, consequently, higher life expectancy, as educated individuals are more likely to engage in healthier behaviors and have access to better healthcare. The Educated condition is shown in @fig-schooling.

5. **Medical Doctors (`pct_doctor`)**: This variable indicates the number of medical doctors per 10,000 people (@fig-doctor). Access to healthcare professionals is a key determinant of health outcomes, and a higher number of doctors generally corresponds with better healthcare services and longer life expectancy.

6. **Nursing and Midwifery Personnel (`pct_nursing`)**: Similar to the number of doctors, this variable represents the number of nursing and midwifery personnel per 10,000 population (@fig-nursing). Nurses play a crucial role in delivering healthcare, and higher availability of nursing staff can contribute to improved health outcomes and increased life expectancy.
...
```{r}
#| label: fig-bmi
#| fig-cap: "Density of BMI by Status, showing a higher concentration of BMI in developed regions around 24, suggesting a potential focus on obesity if the bin width were wider. The distribution for developing regions is fairly evenly distributed, with less concentration in any specific BMI range."
#| echo: false
#| warning: false
#| message: false
...
ggplot(data_filtered, aes(x = BMI, fill = Status)) +
  geom_density(alpha = 0.5) +
  labs(
    title = "BMI Condition by Status (Normal Range)",
    x = "BMI",
    y = "Density"
  ) +
  theme_minimal() +
  scale_fill_manual(values = c("royalblue", "red")) +  # Customize colors for Developed and Developing
  facet_wrap(~Status)
...
Define $y_i$ as the Life Expectancy for country $i$. The factors $x_1$, $x_2$,...
represent socioeconomic predictors such as BMI, Diphtheria, Schooling, TotalExpenditure, pct_doctor, and pct_nursing.

```{=tex}
\begin{align*} 
\text{LifeExpectancy}_{\text{developed}} = &\ \beta_0 + \beta_1 \text{Diphtheria Immunization} + \beta_2 \text{Schooling} + \\
&\ \beta_3 \text{Health Expenditure} + \beta_4 \text{Number of Doctors per 10000} + \epsilon_i
\end{align*}

\begin{align*} 
\text{LifeExpectancy}_{\text{developing}} = &\ \beta_0 + \beta_1 \text{BMI} + \beta_2 \text{Diphtheria Immunization} + \\
&\ \beta_3 \text{Schooling} + \beta_4 \text{Number of Doctors per 10000} + \\
&\ \beta_5 \text{Number of Nurses per 10000} + \epsilon_i
\end{align*}
```
...
**BMI**: Coefficient = 0.040, Standard Error = 0.012
...
#| tbl-cap: "Summary of key model estimates for Developed and Developing counties, including coefficients for predictors like BMI, Diphtheria, Schooling, and TotalExpenditure, pct_doctor, pct_nursing, with standard errors for each estimate. Model performance statistics, such as sample size, R², and adjusted R², are also displayed."
#| echo: false
#| eval: true
#| warning: false
#| message: false
